...
class ParquetDataSourceTest {
  ...
  fun `test ParquetDataSource`() {
    ...
    val parquetSource =
        ParquetDataSource(File("../testdata/", "alltypes_plain.parquet").absolutePath)
  ...
  fun `test ParquetDataSource scan column`() {
    ...
    val idData =
        parquetSource.scan(
            listOf("id", "bool_col", "date_string_col", "string_col", "timestamp_col"))
            .iterator()
    ...
    assertEquals(
        "03/01/09,03/01/09,04/01/09,04/01/09,02/01/09,02/01/09,01/01/09,01/01/09,null",
        dateStrVal.joinToString(","))
